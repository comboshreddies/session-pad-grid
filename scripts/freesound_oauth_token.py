...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        _Handler.oauth_error = (qs.get("error") or [None])[0]
...
def main() -> None:
    ...
    code = _Handler.code
    if _Handler.oauth_error:
        print(f"Freesound denied access: {_Handler.oauth_error}", file=sys.stderr)
        sys.exit(1)
    if not code:
    ...
    try:
        ...
        print(f"Token exchange failed: HTTP {e.code} {e.reason}", file=sys.stderr)
        if err_body:
