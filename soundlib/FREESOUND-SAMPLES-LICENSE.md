...
python3 scripts/freesound_oauth_token.py          # once; redirect http://127.0.0.1:8766/callback
python3 scripts/build_freesound_pack.py         # WAV originals + OAuth token
```

OAuth helper listens on **127.0.0.1:8766** (register the same redirect URI on Freesound) so it does not conflict with `python3 server.py` on **127.0.0.1:8765**.

**401 Invalid token:** `FREESOUND_API_KEY` must be the **Api key** from [apiv2/apply](https://freesound.org/apiv2/apply/) (not client id/secret, not the OAuth access token). Or `unset FREESOUND_API_KEY` and use only `FREESOUND_ACCESS_TOKEN`. OAuth access tokens expire after ~24h — re-run `freesound_oauth_token.py` if needed.
